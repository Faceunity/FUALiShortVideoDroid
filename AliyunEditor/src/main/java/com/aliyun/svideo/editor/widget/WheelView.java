...
//                    Log.d(TAG, "remainder: " + remainder + ", divided: " + divided);
    ...
    private void initData() {
        ...
        for (int i = 0; i < items.size(); i++) {
            views.addView(createView(items.get(i).getText(), i, displayItemCount));
        }
    ...
    private TextView createView(String item, int position, int count) {
...
//            position = divided + offset + 1;
    ...
    private void onSeletedCallBack() {
        if (null != onWheelViewListener) {
            onWheelViewListener.onSelected(selectedIndex - offset, items.get(selectedIndex).getText());
        }
    ...
    public String getSeletedItem() {
        return items.get(selectedIndex).getText();
    }
