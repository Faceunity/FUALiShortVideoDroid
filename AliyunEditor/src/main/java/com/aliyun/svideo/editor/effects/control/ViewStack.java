...
import com.aliyun.svideo.editor.effects.lut.LutFilterChooserView;
...
public class ViewStack {
    ...
    private LutFilterChooserView mLutFilterChooserView;
    ...
    private CaptionChooserPanelView mCaptionChooserPanelView;
    private CaptionChooserView mCaptionChooserView;//字幕
    private PipSettingView mPipSettingView; //画中画
    ...
    public void setActiveIndex(int value) {
        ...
        Log.d(TAG, "setActiveIndex onTabChange: page" + index);
        switch (index) {
        case FILTER:
            // 颜色滤镜
            if (mColorFilterCHoosrView == null) {
                mColorFilterCHoosrView = new ColorFilterChooserView(mContext);
                mColorFilterCHoosrView.setSelectedPos(mEditorService.getEffectIndex(UIEditorPage.FILTER));
            }
            mViewOperator.showBottomView(mColorFilterCHoosrView);
            break;
        case LUT:
            if (mLutFilterChooserView == null) {
                mLutFilterChooserView = new LutFilterChooserView(mContext);
                mLutFilterChooserView.setSelectedPos(mEditorService.getEffectIndex(UIEditorPage.LUT));
            }
            mViewOperator.showBottomView(mLutFilterChooserView);
            break;
        case FILTER_EFFECT:
            // 特效滤镜
        ...
        case SOUND:
            ...
            if (mSoundEffectChooseView == null) {
                ...
                mSoundEffectChooseView.setSelectedPos(mEditorService.getEffectIndex(UIEditorPage.SOUND));
            }
        ...
        case COMPOUND_CAPTION:
            if (mCaptionChooserPanelView == null) {
                mCaptionChooserPanelView = new CaptionChooserPanelView(mContext);
            }
            mCaptionChooserPanelView.setEditorService(mEditorService);
            mCaptionChooserPanelView.setOnEffectChangeListener(mOnEffectChangeListener);
            mCaptionChooserPanelView.setOnEffectActionLister(mOnEffectActionLister);
            setLayoutParams(mCaptionChooserPanelView);
            mCaptionChooserPanelView.addThumbView(rootView.getThumbLineBar());
            mViewOperator.showBottomView(mCaptionChooserPanelView);
            break;
        case AUDIO_MIX:
            // 音乐
            //合拍不允许添加音乐
            ...
            long duration = rootView.getEditor().getPlayerController().getStreamDuration();
        ...
        case TRANSITION: {
            // 转场
            AliyunISourcePartManager clipConstructor = TransitionChooserView.isClipLimit(rootView.getEditor());
        ...
        case PIP:
            if (mPipSettingView == null) {
                mPipSettingView = new PipSettingView(mContext);
            }
            mPipSettingView.setEditorService(mEditorService);
            mPipSettingView.addThumbView(rootView.getThumbLineBar());
            setLayoutParams(mPipSettingView);
            mViewOperator.showBottomView(mPipSettingView);
            break;
        case COVER:
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        case BaseChooser.CAPTION_REQUEST_CODE:
            if (mCaptionChooserPanelView != null) {
                mCaptionChooserPanelView.onActivityResult(requestCode, resultCode, data);
            }
            break;
        default:
            break;
