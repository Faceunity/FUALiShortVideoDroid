...
public abstract class BaseScrollPickerView<T> extends View {
    ...
    private void initAnimator() {
        ...
        mAutoScrollAnimator.setInterpolator(AUTO_SCROLL_INTERPOLATOR);
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        int position;
        int size = mData.size();
        int start = Math.min(length, size);
        if (mDrawAllItem) {
            start = size;
        }
        // 上下两边
     ...
     * @param curr
     * @param end
     * @param rate true:正在滚动, false: 滚动完毕
     */
    private void computeScroll(int curr, int end, boolean rate) {
        if (rate) {
            // 正在滚动
            if (mIsHorizontal) {
    ...
    /**
     * 动画滚动时间比率, animation.getCurrentPlayTime() * 1f / animation.getDuration();
     * 如果 autoScrollRate < 1则正在执行动画中, 反之则动画结束
     */
