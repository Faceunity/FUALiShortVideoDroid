...
//    private volatile byte[] mCameraNV21Byte;
    ...
    private boolean disableCameraData;

    private boolean isActive;

    public void clearCameraData() {
        synchronized (lock) {
            disableCameraData = true;
            cameraData = null;
        }
    }

    public void enableCameraData() {
        synchronized (lock) {
            disableCameraData = false;
        }
    }
    ...
    class CameraData {
        ...
        public CameraData(byte[] bytes, int facing) {
            this.bytes = bytes;
            this.facing = facing;
        }
    }
...
//        deleteFBO();
    ...
    public int onDrawFrame(int texId, int texWidth, int texHeight, float[] transformMatrix) {
        ...
        if (cameraData == null || cameraData.bytes.length != texWidth * texHeight * 3 / 2) {
            Log.e(TAG, "camera nv21 bytes null");
            return texId;
        }

        if (cameraData.facing != facing) {
            onCameraChange(cameraData.facing, 0);
        }
        facing = cameraData.facing;

        return onDrawFrame(cameraData.bytes, texId, texWidth, texHeight);
    }

    private final Object lock = new Object();

    public boolean onPreviewFrame(byte[] data, int width, int height, Camera.CameraInfo cameraInfo) {
        synchronized (lock) {
            if (disableCameraData)
                return true;

            cameraData = new CameraData(data, cameraInfo.facing);
        }
