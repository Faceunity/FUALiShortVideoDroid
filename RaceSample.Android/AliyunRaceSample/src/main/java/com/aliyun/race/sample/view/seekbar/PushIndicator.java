...
public class PushIndicator {
    private final Context mContext;
    private final PushIndicatorSeekBar mSeekBar;
    private final int mWindowWidth;
    private int[] mLocation = new int[2];
    private PushArrowView mIndicatorArrow;
    ...
    public PushIndicator(Context context, PushIndicatorSeekBar seekBar, BuilderParams p) {
    ...
    public void setGap(int dp) {
        mGap = PushIndicatorUtils.dp2px(mContext, dp);
    }

    void initIndicator() {
        if (mBuildParams.mIndicatorType == IndicatorType.CUSTOM) {
            if (mBuildParams.mIndicatorCustomView != null) {
                mIndicatorView = mBuildParams.mIndicatorCustomView;
                ...
                if (progressTextViewId > 0) {
                    ...
                    if (view != null) {
                        if (view instanceof TextView) {
                            ...
                            mIndicatorText.setTextSize(IndicatorUtils.px2sp(mContext, mBuildParams.mIndicatorTextSize));
                            mIndicatorText.setTextColor(mBuildParams.mIndicatorTextColor);
                        } else {
            ...
            if (IndicatorType.CIRCULAR_BUBBLE == mBuildParams.mIndicatorType) {
                mIndicatorView = new PushCircleBubbleView(mBuildParams, checkHolderText());
                ((PushCircleBubbleView) mIndicatorView).setProgress(String.valueOf(mSeekBar.getProgress()));
            } else {
                mIndicatorView = View.inflate(mContext, R.layout.alivc_race_push_isb_indicator, null);
                //container
                mTopContentView = (LinearLayout) mIndicatorView.findViewById(R.id.push_indicator_container);
                //arrow
                mIndicatorArrow = mIndicatorView.findViewById(R.id.push_indicator_arrow);
                mIndicatorArrow.setColor(mBuildParams.mIndicatorColor);
                //progressText
                mIndicatorText = mIndicatorView.findViewById(R.id.push_isb_progress);
                mIndicatorText.setText(String.valueOf(mSeekBar.getProgress()));
                mIndicatorText.setTextSize(IndicatorUtils.px2sp(mContext, mBuildParams.mIndicatorTextSize));
                mIndicatorText.setTextColor(mBuildParams.mIndicatorTextColor);
                ...
                if (mBuildParams.mIndicatorCustomTopContentView != null) {
                    //for the custom indicator top content view, if progress need to show when seeking ,
                    //need a TextView to show progress and this textView 's identify must be progress;
                    int progressTextViewId = mContext.getResources().getIdentifier("isb_progress", "id", mContext.getApplicationContext().getPackageName());
                    View topContentView = mBuildParams.mIndicatorCustomTopContentView;
                    if (progressTextViewId > 0) {
    ...
    String checkHolderText() {
        if (mBuildParams.mSeekBarType == IndicatorSeekBarType.CONTINUOUS || mBuildParams.mSeekBarType == IndicatorSeekBarType.CONTINUOUS_TEXTS_ENDS) {
            String maxString = String.valueOf(mBuildParams.mMax);
            String minString = String.valueOf(mBuildParams.mMin);
            ...
            if (mBuildParams.mTextArray != null) {
                ...
                for (CharSequence c : mBuildParams.mTextArray) {
    ...
    private GradientDrawable getGradientDrawable() {
        ...
        if (mBuildParams.mIndicatorType == IndicatorType.RECTANGLE_ROUNDED_CORNER) {
        ...
        tvDrawable.setColor(mBuildParams.mIndicatorColor);
        return tvDrawable;
    }
    ...
    private void adjustArrow(float touchX) {
        if (mBuildParams.mIndicatorType == IndicatorType.CUSTOM || mBuildParams.mIndicatorType == IndicatorType.CIRCULAR_BUBBLE) {
            return;
        }
