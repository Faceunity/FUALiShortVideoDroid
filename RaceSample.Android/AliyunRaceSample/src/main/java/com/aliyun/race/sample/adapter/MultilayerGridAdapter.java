...
public class MultilayerGridAdapter extends BaseAdapter {
    ...
    private List<ScenesModel> mListData;
    ...
    public MultilayerGridAdapter(Context context, List<ScenesModel> listData, int mIndex, int mPagerSize) {
        ...
        mInflater = LayoutInflater.from(this.mContext.get());
    }

    /**
     * 先判断数据集的大小是否足够显示满本页？mListData.size() > (mIndex + 1)*mPagerSize
     * 如果满足，则此页就显示最大数量mPagerSize的个数
     * 如果不够显示每页的最大数量，那么剩下几个就显示几个 (mListData.size() - mIndex*mPagerSize)
     */
    @Override
    public int getCount() {
        return mListData.size() > (mIndex + 1) * mPagerSize ? mPagerSize : (mListData.size() - mIndex * mPagerSize);
    }

    @Override
    public Object getItem(int position) {
        return mListData.get(position + mIndex * mPagerSize);
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            convertView = mInflater.inflate(R.layout.alivc_race_home_item_layout, parent, false);
            ...
            int width = (ScreenUtils.getWidth(mContext.get()) - 100) / 2;
        ...
        ScenesModel scenesModel = mListData.get(pos);
